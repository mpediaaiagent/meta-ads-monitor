...
// conversions over the same 10-day window the adset row itself is built from.
export async function onRequestGet(context) {
  ...
  let sql =
    `SELECT ad_name, ad_status, ad_created_date, spend_5d, conv_5d, spend_10d, conv_10d, daily_json, report_date
     FROM ad_snapshots
  ...
  return Response.json(
    { ads, dates, reportDate: results.length ? results[0].report_date : null },
