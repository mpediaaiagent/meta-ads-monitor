...
test("the spend-as-CPP rule reaches every CPP comparison, not just the thresholds", () => {
  // one successful ad, cheap: its day-2 CPP line is 100 + 10% = 110, first-purchase limit 100
  const cohort = [{ adId: "A", campaignName: "trubuddy", daily: dailyFromCumulative([[100, 1], [200, 2], ...Array(9).fill([300, 3])]) }];
  const bm = buildBenchmarks(cohort).products.trubuddy;
  assert.equal(bm.days[1].upperBound, 110);

  // ₹150 on day 2 with no purchase: over the ₹100 spend limit, so that is what is reported
  const overBoth = evaluateAd(dailyFromCumulative([[50, 0], [150, 0]]), bm);
  assert.equal(overBoth.days[1].cumCpp, 150, "spend is the CPP");
  assert.equal(overBoth.verdict, VERDICT.PAUSE);

  // an ad past incubation with no purchase in the window: the window threshold now bites too
  const w = judgeWindow({ spend: 500, purchases: 0 }, bm, 280);
  assert.equal(w.cpp, 500);
  assert.equal(w.verdict, VERDICT.PAUSE);
});
