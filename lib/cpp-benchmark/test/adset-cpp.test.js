...
test("with no purchase the adset's spend is its CPP, so this rule applies to it too", () => {
  const r = adsetCpp([live(dailyFromCumulative([[900, 0], [1800, 0], [2700, 0]]))], TB, CFG);
  assert.equal(r.cumCpp, 2700, "₹2,700 spent for nothing reads as a ₹2,700 CPP");
  assert.equal(r.verdict, VERDICT.PAUSE, "far above the day-3 line of 286");
  assert.equal(r.reason, REASON.ADSET_CPP_ABOVE_BENCHMARK);

  // and a small spend with no purchase is still within the line
  const small = adsetCpp([live(dailyFromCumulative([[80, 0], [160, 0], [240, 0]]))], TB, CFG);
  assert.equal(small.cumCpp, 240);
  assert.equal(small.verdict, VERDICT.KEEP);
});
...
test("an unconverted adset faces both rules, and either can pause it", () => {
  // ₹700 with no purchase: past the ₹660 spend limit AND past the day-3 CPP line
  const noBuy = [live(dailyFromCumulative([[10, 0], [20, 0], [700, 0]]))];
  assert.equal(adsetCpp(noBuy, SP, CFG).verdict, VERDICT.PAUSE, "spend is the CPP, so this rule applies now");
  assert.equal(adsetSpendBeforeFirstPurchase(noBuy, SP, CFG).verdict, VERDICT.PAUSE);

  // once it converts, only the CPP rule is left — the spend rule stands down for good
