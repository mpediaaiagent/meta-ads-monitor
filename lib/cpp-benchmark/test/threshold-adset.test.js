...
test("past day 9, no purchase in the window: both the spend limit and the threshold apply", () => {
  // the window's spend is its CPP, so ₹700 for nothing is a ₹700 CPP — past the ₹280 threshold
  // even though it is within the ₹700 first-purchase limit
  const both = judgeWindow({ spend: 700, purchases: 0 }, BM, 280);
  assert.equal(both.cpp, 700, "spend became the CPP");
  assert.equal(both.verdict, VERDICT.PAUSE);
  assert.equal(both.reason, REASON.WINDOW_CPP_ABOVE_THRESHOLD, "within the spend limit, but over the threshold");

  // past the spend limit too: that is the more specific signal, so it is the one named
  ...
  // inside both lines is still Keep
  const ok = judgeWindow({ spend: 200, purchases: 0 }, BM, 280);
  assert.equal(ok.verdict, VERDICT.KEEP);
  assert.equal(ok.reason, REASON.WITHIN_FIRST_PURCHASE_LIMIT);
});
...
test("past day 9 with nothing to compare against is Keep / no_benchmark", () => {
  ...
  // no benchmark AND no threshold: nothing to say
  assert.equal(judgeWindow({ spend: 9999, purchases: 0 }, null, null).reason, REASON.NO_BENCHMARK);
  assert.equal(judgeWindow({ spend: 9999, purchases: 0 }, null, null).verdict, VERDICT.KEEP);
  // with only a threshold, the spend-as-CPP still gets measured against it
  assert.equal(judgeWindow({ spend: 9999, purchases: 0 }, null, 280).verdict, VERDICT.PAUSE);
});
...
test("adset first purchase: day 1 is the first day any of its ads spent, and every ad's purchases count", () => {
  ...
  assert.deepEqual(adsetFirstPurchase(ads, SLOWEST_3, CFG), {
...
test("a first purchase on the slowest ad's day is in time", () => {
  ...
  const r = adsetFirstPurchase(ads, SLOWEST_3, CFG);
...
test("with no purchase yet, the adset is Pause once it has run more days than the slowest ad took", () => {
  ...
  const young = adsetFirstPurchase(ads, SLOWEST_3, CFG);
  assert.equal(young.ageDays, 2);
  assert.equal(young.verdict, null, "day 2 is immune, so no adset-level verdict at all");
  assert.equal(young.reason, REASON.IMMUNE_EARLY_DAYS);
  assert.equal(adsetFirstPurchase(ads, SLOWEST_3, CFG, { through: "2026-09-04" }).verdict, VERDICT.KEEP, "day 3: on the line");
  // days without delivery at the end still count as waiting
  const late = adsetFirstPurchase(ads, SLOWEST_3, CFG, { through: "2026-09-05" });
...
test("no limit is Keep / no_benchmark, and an adset that never spent has no verdict", () => {
  const r = adsetFirstPurchase([{ daily: [row("2026-09-02", 5000)] }], null, CFG, { through: "2026-09-20" });
  assert.equal(r.verdict, VERDICT.KEEP);
  assert.equal(r.reason, REASON.NO_BENCHMARK);
  assert.equal(adsetFirstPurchase([{ daily: [row("2026-09-02", 0)] }], SLOWEST_3, CFG).verdict, null);
  assert.equal(adsetFirstPurchase([], SLOWEST_3, CFG).firstSpendDate, null);
});
