...
test("the edited spend-with-no-purchase threshold moves the Pause line for unconverted ads", () => {
  ...
  // and a looser one keeps an ad alive that the derived limit would have paused
  const loose = buildBenchmarks(COHORT, { maxSpendNoPurchaseByProduct: { trubuddy: 1200 } }).products.trubuddy;
  assert.equal(evaluateAd(dailyFromCumulative([[900, 0]]), loose).verdict, VERDICT.KEEP);
  assert.equal(evaluateAd(dailyFromCumulative([[900, 0]]), TB).verdict, VERDICT.PAUSE);
});
