...
test("day 1 is immune: its performance never decides a verdict", () => {
  // ₹9,999 on day 1 would be far past every limit, and it is still Keep
  const r = evaluateAd(dailyFromCumulative([[9999, 0]]), TB);
  assert.equal(r.verdict, VERDICT.KEEP);
  assert.equal(r.reason, REASON.IMMUNE_EARLY_DAYS);
  assert.equal(r.days[0].cumSpend, 9999, "the day is still in the grid with its real numbers");
});

test("no purchase yet: the spend is the CPP, so BOTH the spend limit and the day's CPP line apply", () => {
  // day-2 CPP line is 352, day-3 is 330; the first-purchase spend limit is 700.
  // ₹300 on day 2 is inside both; ₹340 on day 3 is inside the spend limit but over the CPP line.
  const r = evaluateAd(dailyFromCumulative([[100, 0], [300, 0], [340, 0]]), TB);
  assert.deepEqual(verdicts(r), [VERDICT.KEEP, VERDICT.KEEP, VERDICT.PAUSE]);
  assert.deepEqual(r.days.map((d) => d.reason), [REASON.IMMUNE_EARLY_DAYS, REASON.WITHIN_FIRST_PURCHASE_LIMIT, REASON.CPP_ABOVE_BENCHMARK]);
  assert.equal(r.days[2].cumCpp, 340, "₹340 spent for nothing reads as a ₹340 CPP");

  // past the spend limit as well: that is the more specific signal, so it is the one reported
  const worse = evaluateAd(dailyFromCumulative([[100, 0], [300, 0], [701, 0]]), TB);
  assert.equal(worse.reason, REASON.SPEND_WITHOUT_PURCHASE);
  assert.deepEqual(worse.days[2].benchmark, { kind: BENCHMARK_KIND.FIRST_PURCHASE_LIMIT, limit: 700, sampleSize: 3 });
});

test("the edited spend-with-no-purchase threshold moves the Pause line for unconverted ads", () => {
  // ₹300 on day 2 is within the day-2 CPP line of 352, so the spend threshold is what decides.
  // The dashboard's ₹250 threshold is stricter than the derived ₹700 and pauses it.
  const strict = buildBenchmarks(COHORT, { maxSpendNoPurchaseByProduct: { trubuddy: 250 } }).products.trubuddy;
  const r = evaluateAd(dailyFromCumulative([[100, 0], [300, 0]]), strict);
  assert.equal(r.verdict, VERDICT.PAUSE);
  assert.equal(r.reason, REASON.SPEND_WITHOUT_PURCHASE);
  assert.equal(r.days[1].benchmark.limit, 250);

  // a looser threshold leaves the same ad alone, because ₹300 is inside the day-2 CPP line too
  const loose = buildBenchmarks(COHORT, { maxSpendNoPurchaseByProduct: { trubuddy: 1200 } }).products.trubuddy;
  assert.equal(evaluateAd(dailyFromCumulative([[100, 0], [300, 0]]), loose).verdict, VERDICT.KEEP);
});

test("the spend-with-no-purchase threshold doesn't touch an ad that has purchases", () => {
  // ₹100 CPP is well inside the CPP benchmark; a tiny spend threshold must not pause it
  const strict = buildBenchmarks(COHORT, { maxSpendNoPurchaseByProduct: { trubuddy: 50 } }).products.trubuddy;
  const r = evaluateAd(dailyWithCpp([100, 100, 100], RAMP), strict);
  assert.deepEqual(verdicts(r), [VERDICT.KEEP, VERDICT.KEEP, VERDICT.KEEP]);
  assert.equal(r.days[2].benchmark.kind, BENCHMARK_KIND.CPP);
});

test("the first-purchase limit has no margin on it", () => {
  const r = evaluateAd(dailyFromCumulative([[300, 0], [710, 0]]), TB);
...
test("nothing to compare against means Keep, labelled no_benchmark", () => {
  // two days, so the verdict comes from day 2 rather than the immune day 1
  const empty = buildBenchmarks([]).products.gulu;
  const noPurchase = evaluateAd(dailyFromCumulative([[2500, 0], [5000, 0]]), empty);
  const withPurchase = evaluateAd(dailyWithCpp([5000, 5000], [1, 1]), empty);
  const noProduct = evaluateAd(dailyWithCpp([5000, 5000], [1, 1]), null);
...
test("a day with a purchase but no successful ad that had one by then is Keep / no_benchmark", () => {
  ...
  assert.deepEqual(r.days.map((d) => d.reason), [REASON.IMMUNE_EARLY_DAYS, REASON.NO_BENCHMARK, REASON.CPP_ABOVE_BENCHMARK]);
});
...
test("margin and incubation length are settings", () => {
  ...
  const short = evaluateAd(dailyWithCpp([300, 300, 300, 300, 300, 300], RAMP), TB, { incubationMaxDay: 5, successMinDays: 6, successCppDay: 5 });
