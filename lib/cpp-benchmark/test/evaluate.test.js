...
test("no purchase yet: Keep while spend is within the first-purchase limit, Pause once it goes past", () => {
  ...
  assert.deepEqual(r.days.map((d) => d.reason), [REASON.IMMUNE_EARLY_DAYS, REASON.WITHIN_FIRST_PURCHASE_LIMIT, REASON.SPEND_WITHOUT_PURCHASE]);
...
test("the edited spend-with-no-purchase threshold moves the Pause line for unconverted ads", () => {
  // same cohort, but the dashboard's threshold (₹400) is stricter than the derived ₹700
  const strict = buildBenchmarks(COHORT, { maxSpendNoPurchaseByProduct: { trubuddy: 400 } }).products.trubuddy;
  const r = evaluateAd(dailyFromCumulative([[300, 0], [400, 0], [400.01, 0]]), strict);
  assert.deepEqual(verdicts(r), [VERDICT.KEEP, VERDICT.KEEP, VERDICT.PAUSE]);
  assert.equal(r.days[2].benchmark.limit, 400);

  // and a looser one keeps an ad alive that the derived limit would have paused (day 2: past immunity)
  const loose = buildBenchmarks(COHORT, { maxSpendNoPurchaseByProduct: { trubuddy: 1200 } }).products.trubuddy;
  const twoDays = dailyFromCumulative([[450, 0], [900, 0]]);
  assert.equal(evaluateAd(twoDays, loose).verdict, VERDICT.KEEP);
  assert.equal(evaluateAd(twoDays, TB).verdict, VERDICT.PAUSE);
});
