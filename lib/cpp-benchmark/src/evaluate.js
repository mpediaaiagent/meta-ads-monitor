...
export function judgeWindow(totals, benchmark, maxCpp) {
  ...
  const base = { spend: round2(spend), purchases, cpp: purchases > 0 ? round2(spend / purchases) : null };

  if (purchases === 0) {
    const limit = benchmark?.firstPurchaseLimit?.value ?? null;
    const detail = { kind: BENCHMARK_KIND.FIRST_PURCHASE_LIMIT, limit, sampleSize: benchmark?.firstPurchaseLimit?.sampleSize ?? 0 };
    if (limit === null) return { ...base, verdict: VERDICT.KEEP, reason: REASON.NO_BENCHMARK, benchmark: detail };
    const pause = above(spend, limit);
    return {
      ...base,
      verdict: pause ? VERDICT.PAUSE : VERDICT.KEEP,
      reason: pause ? REASON.SPEND_WITHOUT_PURCHASE : REASON.WITHIN_FIRST_PURCHASE_LIMIT,
  ...
  const pause = above(spend / purchases, maxCpp);
  return {
    ...base,
    verdict: pause ? VERDICT.PAUSE : VERDICT.KEEP,
    reason: pause ? REASON.WINDOW_CPP_ABOVE_THRESHOLD : REASON.WITHIN_WINDOW_THRESHOLD,
    benchmark: detail,
  };
}
