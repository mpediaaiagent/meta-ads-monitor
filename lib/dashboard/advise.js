...
export async function loadAdviseContext(db) {
  ...
  const cfg = resolveConfig({ maxCppByProduct, maxSpendNoPurchaseByProduct, productDayThresholds: DEFAULT_DAY_THRESHOLDS });
...
export function judgeAdset(ads, dates, ctx, product) {
  ...
  /*
   * The product's own day-wise threshold overrules the 3-month benchmark. The benchmark is drawn
   * from history and can be stricter than the number the business actually runs to, so an adset
   * inside its product's threshold is Keep whatever the benchmark said. Only a rescue: being above
   * the threshold does not by itself pause anything, it just lets the benchmark's flag stand.
   *
   * Products with no schedule (adi anku, educator program) have nothing to rescue them, which is
   * the old behaviour.
   */
  const threshold = adsetThresholdCheck(ads, dates, ctx, product);
  const rescued = threshold.checked && threshold.verdict === VERDICT.KEEP;

  const costly = cpp.checked && cpp.verdict === VERDICT.PAUSE && !rescued;
  const overspent = spend.checked && spend.verdict === VERDICT.PAUSE && !rescued;
  ...
  return {
    ...
    /** true when the product threshold overruled a benchmark flag back to Keep */
    rescued: rescued && ((cpp.checked && cpp.verdict === VERDICT.PAUSE) || (spend.checked && spend.verdict === VERDICT.PAUSE)),
  };
}

/** The adset's latest day against its product's own day-wise threshold. */
function adsetThresholdCheck(ads, dates, ctx, product) {
  const blocked = adsetWindowGuard(ads, dates, product);
  if (blocked) return blocked;
  return adsetProductThreshold(adsetSeries(ads), ctx.cfg.productDayThresholds?.[product] ?? null, ctx.cfg, {
...
function firstPurchaseCheck(ads, dates, ctx, product) {
  const blocked = adsetWindowGuard(ads, dates, product);
  if (blocked) return blocked;
  const out = adsetFirstPurchase(adsetSeries(ads), ctx.products[product], ctx.cfg, { through: dates[dates.length - 1] });
  return { checked: out.verdict !== null, notChecked: out.verdict === null ? out.reason : null, ...out };
}
