...
export function judgeAdset(ads, dates, ctx, product) {
  ...
  return {
    verdict: slow || costly || roll.verdict === VERDICT.PAUSE ? VERDICT.PAUSE : roll.verdict,
    basis,
...
function adsetCppCheck(ads, dates, ctx, product) {
  ...
  // day 1 must be real: the same guard, and the same reason, as firstPurchaseCheck
  ...
  const out = adsetCpp(series, ctx.adsetProducts?.[product] ?? null, ctx.cfg, { through: dates[dates.length - 1] });
