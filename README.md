...
There is no test suite and no build step. To verify a frontend change, render the page and interact with it — do not just eyeball the CSS. The quickest loop is to copy `public/index.html` somewhere temporary, stub `window.fetch` with sample `snapshots` / `thresholds` responses, and serve that directory over plain HTTP (e.g. `npx http-server`); that exercises layout, sticky columns, the panel toggle and the save flow without needing D1.
